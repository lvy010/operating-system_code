...
/*
 * loop 设备主结构体，描述一个 loop 设备的所有状态和资源
 */
struct loop_device {
	int		lo_number;
	loff_t		lo_offset;
...
#define LOOP_IDLE_WORKER_TIMEOUT (60 * HZ) // 空闲 worker 超时时间
#define LOOP_DEFAULT_HW_Q_DEPTH 128        // 默认硬件队列深度

static DEFINE_IDR(loop_index_idr);         // loop 设备 idr 索引
static DEFINE_MUTEX(loop_ctl_mutex);       // 控制全局 loop 设备的互斥锁
static DEFINE_MUTEX(loop_validate_mutex);  // 校验 loop 文件的全局锁

/**
 * loop_global_lock_killable() - 获取全局和设备锁，保证 loop_validate_file() 安全
 * @lo: loop 设备
 * @global: 是否需要全局锁
 * 返回0表示成功，-EINTR表示被信号中断
 */
...
/**
 * loop_global_unlock() - 释放 loop_global_lock_killable() 获取的锁
 */
...
static int max_part;      // 每个 loop 设备最大分区数
static int part_shift;    // 分区位移
...
/**
 * loop_set_size() - 设置 loop 设备容量并通知用户空间
 * @lo: loop 设备
 * @size: 新容量
 */
...
static void loop_clear_limits(struct loop_device *lo, int mode)
{
	...
	// 注意：此处未冻结队列，理论上有风险，但在queue_rq上下文只能这样
	queue_limits_commit_update(lo->lo_queue, &lim);
}

/*
 * 处理 fallocate/丢弃/写零请求
 * 通过fallocate实现空间回收或写零
 */
...
/*
 * loop_worker 结构体，负责处理 I/O 命令的工作线程
 */
struct loop_worker {
    ...
    struct loop_device *lo;         // 所属 loop 设备
    struct cgroup_subsys_state *blkcg_css; // blkcg 状态
    unsigned long last_ran_at;      // 上次运行时间
};

/*
 * loop_workfn: worker线程的主处理函数，处理cmd_list中的所有命令
 */
...
/*
 * loop_rootcg_workfn: 根cgroup worker的处理函数
 */
...
/*
 * loop_queue_rq: loop设备的主队列请求入口
 * 负责将请求转为loop_cmd并分发到worker
 */
...
/*
 * loop_handle_cmd: 实际执行I/O命令，处理读写/丢弃/flush等
 * 支持cgroup和memcg上下文切换
 */
...
/*
 * loop_process_work: 处理worker的命令队列
 * 支持空闲worker回收
 */
            ...
            struct list_head *cmd_list, struct loop_device *lo)
{
    ...
    /*
     * 如果没有待处理命令且worker不会再次运行，则加入空闲列表
     * 便于后续定时回收
     */
...
static const struct blk_mq_ops loop_mq_ops = {
    .queue_rq       = loop_queue_rq,      // 队列请求处理
    .complete       = lo_complete_rq,     // 请求完成回调
};
